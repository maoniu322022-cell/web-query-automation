...
def build_row(phone, search, limiter):
    ...
    if search.get("__error__"):
        row["status"] = "error"
        error_msg = search['__error__']
        row["note"] = f"search error: {error_msg}"
        print(f"[ERROR] {phone}: {error_msg}")  # 详细错误输出
        return row, None
...
def read_api_key(path):
    ...
    with open(path, encoding="utf-8-sig") as f:
        for line in f:
            ...
            if "=" in s:
                s = s.split("=", 1)[1].strip()
            key = s.strip().strip('"').strip("'")
            print(f"[info] API Key loaded: {key[:10]}...")
            return key
